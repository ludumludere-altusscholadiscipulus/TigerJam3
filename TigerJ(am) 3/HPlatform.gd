...
func _ready():
	tween.interpolate_property(self,"position",self.position,Vector2(x_distance,self.position.y),time,Tween.TRANS_SINE,Tween.EASE_IN_OUT,idle_duration)
	tween.interpolate_property(self,"position",Vector2(x_distance,self.position.y),self.position,time,Tween.TRANS_SINE,Tween.EASE_IN_OUT,time+idle_duration)
	tween.start()
